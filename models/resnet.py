...
class FocalBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        if stride != 1 or in_channels != out_channels * FocalBlock.expansion:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_channels, out_channels * FocalBlock.expansion, stride=stride, kernel_size=1, bias=False),
                nn.BatchNorm2d(out_channels * FocalBlock.expansion)
            )
